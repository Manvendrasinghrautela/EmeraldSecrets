...
class AffiliateProgram(models.Model):
    ...
    # ✅ UPDATED: Commission Settings - NOW 5% commission
    commission_rate = models.DecimalField(max_digits=5, decimal_places=2, default=5.00)  # ✅ 5% (was 2%)
    
    # Withdrawal Settings - ₹1000 minimum
...
class AffiliateUser(models.Model):
    """Affiliate Program Member - Track users who join"""
    STATUS_CHOICES = [
    ...
    # Unique affiliate code (for referral link)
    ...
    # Check if user can withdraw (₹1000 minimum)
    @property
...
Commission Rate: {self.program.commission_rate}% ✅ NOW 5%!
...
class AffiliateClick(models.Model):
    """Track affiliate link clicks - Track referral activity"""
...
class AffiliateOrder(models.Model):
    """Track orders from affiliates - ✅ 5% commission calculation"""
    STATUS_CHOICES = [
    ...
    # ✅ Track order amount and 5% commission
    order_amount = models.DecimalField(max_digits=15, decimal_places=2)
    commission_rate = models.DecimalField(max_digits=5, decimal_places=2)  # Store rate at time of order (5.00)
    commission_amount = models.DecimalField(max_digits=15, decimal_places=2)  # ✅ 5% of order_amount
    ...
    def __str__(self):
        ...
        return f"{self.affiliate.affiliate_code} - Order #{order_num} - ₹{self.order_amount} (5% = ₹{self.commission_amount})"
    ...
    def complete(self):
        """Complete order - Add commission to earnings"""
        if self.status in ['pending', 'confirmed']:
            ...
            AffiliateTransaction.objects.create(
                ...
                description=f"5% Commission from Order #{self.order.order_number if self.order else 'N/A'}",
                related_order=self,
...
class AffiliateWithdrawal(models.Model):
    """Track affiliate withdrawals - ₹1000 minimum"""
    STATUS_CHOICES = [
        ('pending', 'Pending'),
        ('approved', 'Approved'),
    ...
    def mark_paid(self):
        """Mark withdrawal as paid - Update withdrawn amount"""
...
class AffiliateTransaction(models.Model):
    """Store ALL transactions for easy tracking"""
    TRANSACTION_TYPES = [
...
# ============================================================================
# HELPER FUNCTION - Calculate 5% commission
# ============================================================================

def calculate_affiliate_commission(order_amount):
    """
    Calculate 5% commission on order amount
    
    Args:
        order_amount: Decimal - Total order amount
    
    Returns:
        Decimal - 5% of order_amount
    
    Example:
        >>> calculate_affiliate_commission(Decimal('1000'))
        Decimal('50.00')
    """
